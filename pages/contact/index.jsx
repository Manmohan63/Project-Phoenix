  ...
  <div className={`pt-16 pb-20 px-20 sm:pt-4 sm:pb-6 sm:px-6`}>
    <div className="flex flex-col justify-center border-4 border-current p-5 sm:p-2 h-full rounded-lg backdrop-blur-sm">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col justify-center items-center my-4">
        ...
        <input placeholder='Your Name' className='text__black p-1.5 my-1.5 w-3/5 bg-transparent border-2 border-main rounded-md sm:w-full' name="name" {...register('name', { required: true })}/>
        {errors.name && <span>This field is required</span>}

        <input placeholder='Your Email' className='text__black p-1.5 my-1.5 w-3/5 bg-transparent border-2 border-main rounded-md sm:w-full' name="email" type="email" {...register('email', { required: true })} />
        {errors.email && <span>This field is required</span>}

        <input placeholder='Your Subject' className='text__black p-1.5 my-1.5 w-3/5 bg-transparent border-2 border-main rounded-md sm:w-full' name="subject" {...register('subject', { required: true })} />
        {errors.subject && <span>This field is required</span>}

        <textarea placeholder='Your Message' className='text__black p-1.5 resize-none my-1.5 w-3/5 border-2 border-main bg-transparent rounded-md sm:w-full' name="message" {...register('message', { required: true })} />
